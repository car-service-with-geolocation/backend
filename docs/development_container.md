...
Этот контейнер - это независимый сервер, который использует Werkzeug и Django для функционирования. На этом сервере работает версия приложения backed, которая размещена в локальной директории компьютера. Любые изменения в исходном коде непосредственно влияют на функционирование контейнера.
...
Для запуска, перезапуска и останова контейнера рекомендуется использовать команды Docker Compose:
